...
class Client(Base_Client):
    def __init__(self, client_dict, args):
        ...
        self.optimizer = torch.optim.SGD(self.model.parameters(), lr=self.args.lr, momentum=0.9 if args.momentum else 0,# momentum=0.9, nesterov=True,
...
class Server(Base_Server):
    ...
    def operations(self, client_info):
        ...
        self.model.clf.margin = self.sim
        # if self.args.save_client:
        #     for client in client_info:
        #         torch.save(client['weights'], '{}/client_{}.pt'.format(self.save_path, client['client_index']))
        self.round += 1
        return [[self.model.cpu().state_dict(), self.sim] for _ in range(self.args.thread_number)]
    ...
    def retrain(self):
        ...
        linear_g = SoftmaxMargin(in_features_dict[self.args.net], self.num_classes, margin=self.sim).to(self.device)
