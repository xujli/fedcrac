...
class Server(Base_Server):
    def __init__(self, server_dict, args):
        ...
        self.coef = torch.tensor(args.mu)
    ...
    def get_mean_var(self, client_info):
        ...
        for i in range(self.num_classes): # L2 war
            sim[i, :] = torch.mean(torch.pow(self.mean[i] - self.mean, 2), dim=1) + torch.mean(torch.pow(torch.sqrt(self.var[i]) - torch.sqrt(self.var), 2), dim=1)
            # sim[i, :] = F.cosine_similarity(self.mean[i], \
            #     self.mean, dim=1)

        sim = sim / sim.max(dim=1)[0]
        print(sim)
        return sim
    ...
    def operations(self, client_info):
        ...
        self.model.clf.margin = deepcopy(self.sim) 
        # if self.args.save_client:
        #     for client in client_info:
        #         torch.save(client['weights'], '{}/client_{}.pt'.format(self.save_path, client['client_index']))
        self.round += 1
        return [[self.model.cpu().state_dict(), deepcopy(self.sim)] for _ in range(self.args.thread_number)]
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            wandb_dict[self.args.method + "_averagemargin".format(self.args.mu)] = (self.sim.sum().numpy()) / (self.num_classes - 1) / self.num_classes 
            ...
            if self.round == self.args.comm_round:
                ...
                table = wandb.Table(data=pd.DataFrame({'{} CSAM'.format(self.args.method): np.reshape(self.sim.cpu().numpy(), -1)}))
                wandb.log({'{} CSAM'.format(self.args.method): table})
