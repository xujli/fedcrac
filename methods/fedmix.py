    ...
    def train(self):
        ...
        for epoch in range(self.args.epochs):
            ...
            for batch_idx, (images, labels) in enumerate(self.train_dataloader):
                ...
                inputX = (1 - self.lam) * images
                inputX.requires_grad_()
                log_probs = self.model(inputX)
                loss1 = (1 - self.lam) * self.criterion(log_probs, labels)
                loss_soft = self.lam * self.criterion(log_probs, labels_2)

                jacobian = torch.autograd.grad(outputs=loss1, inputs=inputX, retain_graph=True)[0].view(
                    batch_size, 1, -1)
                loss2 = self.lam * torch.mean(torch.bmm(jacobian, images_2.view(batch_size, -1, 1)))

                loss = loss1 + loss2 + loss_soft
